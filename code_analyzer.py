...
class CodeAnalyzer:
    ...
    def var_declarations(line):
        ...
        common_type = m[1]
        s = m[2]
        start, end = m.span(2)#эта строка сохраняет начальную и конечную позиции второй группы захвата регулярного выражения
        l = l[start:end]
        pos = 0
    ...
    @staticmethod       
    def preprocess_line(line):# даем каждому символу метку принадлежности к скобке, коменту и т д.... Возращает см рис.1
        line = line.strip()
        ...
        for c in line:
            ...
            elif br in "\"\'": # это строка "' (два символа подряд)
                if c == br and prevc != "\\":
                    brpop = brs.pop()
                elif c == "\\" and prevc == "\\":
